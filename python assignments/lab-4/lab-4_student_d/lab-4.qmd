...
Is there any missing data? Yes, as many of the variables contain missing values, mainly for features that are not present in some houses.
:::
...
norm_recipe <- 
  recipes::recipe(Sale_Price ~ Longitude + Latitude + Lot_Area + Neighborhood + Year_Sold,
                  data = ames_train) %>%
...
train_resamples \<- \_\_\_ %\>% rsample::bootstraps()
...
best_model_workflow <- 
  ...
  tune::finalize_workflow(
    tibble::tibble(__ = __, __ = __) # enter the name and value of the best-fit parameters from exercise 8
  ) 
```
:::
