...
def main(lab_number: int) -> None:
    ...
    as a UTF-8 CSV with columns: ``Student``, ``Total``, ``Model_Total``,
    ``OverallComment``, ``Q1``–``Q{Q_COUNT}``, and
    ``Q1_feedback``–``Q{Q_COUNT}_feedback``.
    ...
    for path in sorted(base_dir.rglob(f"lab-{lab_number}.qmd")):
        ...
        try:
            ...
            # model-returned total, which can drift.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Grade all student submissions for a lab using the Anthropic Claude API."
    )
    parser.add_argument(
        "--lab-number", "-n",
        type=int,
