...
row_A_after_6 <- P6[1, ]              # row for "New Customer"
prob_still_customer_6 <- sum(row_A_after_6[1:3])   # A+B+C
prob_still_customer_6
```

The probability that a customer that has just signed up is still a customer after 6 months is \_74.85\_\_%
:::
...
In the steady state, the probability of being a current customer is \_23.08\_\_%
:::
...
pi(y)/pi(x) = lambda / (x+1)\

q is symmetric\

alpha = min(1, lambda/(x+1))
...
alpha = min(1, x/lambda)

**3. Case x = 0, y = 1:**\

q(1\|0) = 1 and q(0\|1) = 1/2\

pi(1)/pi(0) = lambda\

alpha = min(1, lambda/2)

**4. Case x = 1, y = 0:**\

q(0\|1) = 1/2 and q(1\|0) = 1\

pi(0)/pi(1) = 1/lambda\

alpha = min(1, 2/lambda)
:::
...
sample_q <- quantile(samples, probs = probs)
theory_q <- qpois(probs, lambda = lambda)

cbind(prob = probs, sample_q = sample_q, theory_q = theory_q)
```
:::
...
\_\_76.51\_❓ percent of good loans are paid in full after 20 years
...
\_\_19.6\_❓ percent of risky loans are paid in full after 20 years
:::
...
How do the two covariance calculations compare: \_\_\_\_\_The two covariance calculations are almost identical for all sample sizes. Any differences are extremely small numerical rounding errors, so both methods produce the same covariance in practice.\_\_\_\_\_\_
:::
