...
-   set your username and email with
    -   `usethis::use_git_config( user.name = "Sindy Martinez", user.email = "martin4l@uwindsor.ca")`
:::
...
Missing <- skimr::skim(dat) |> as.tibble()
Missing
  

dplyr::filter(Missing, n_missing > 0)
```

Is there any missing data? [No, there is not any missing data]{.underline}
:::
...
[The training dataset es 2197 observations that represent the 0.7498 of the total data.]{.underline}
:::
...
norm_recipe <- 
  recipes::recipe(Sale_Price ~ Longitude + Latitude + Lot_Area + Neighborhood + Year_Sold, data = dat_train) %>%
  ...
  prep_recipe <- recipes::prep(norm_recipe, training = dat_train) %>% broom::tidy()
...
lm_mod_glmnet <- parsnip::linear_reg(penalty = tune(),
                                     mixture = tune()
                                     ) %>%
                 parsnip::set_mode("regression") %>%
...
lm_mod_rforest <- parsnip::rand_forest(min_n = tune(),
                                       trees = tune()
                                       ) %>%
                  parsnip::set_mode("regression") %>%
...
train_resamples <- dat_train %>% rsample::bootstraps()
...
all_workflows <- 
  workflowsets::workflow_set(
    preproc = list("base_recip" = norm_recipe),
    models = list("lm_base" = lm_mod_base, 
                  "elastic_net" = lm_mod_glmnet, 
                  "random_forest" = lm_mod_rforest
                  )
  )
...
best_model_workflow <- 
  all_workflows %>% 
  workflowsets::extract_workflow("base_recip_random_forest")
```
:::
...
What is the ratio of the OOB prediction errors (MSE): test/train? the OBB prediction error (MSE) for test is 0.01930128 and for train is 0.01495494
:::
