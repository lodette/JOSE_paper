...
set.seed(8740)
train_resamples <- bootstraps(ames_train)
train_resamples
```
:::
...
#The results from Exercise 9 do not match the results from Exercise 8.
#This is expected because the code in Exercise 8 sorts RMSE in descending order, so it selects the worst-performing configuration, while rank_results() selects the lowest RMSE, i.e. the best-performing configuration.
```
:::
...
best_model_workflow <- 
  all_workflows %>% 
  workflowsets::extract_workflow("norm_forest")
...
best_model_workflow <- 
  ...
  tune::finalize_workflow(
    tibble::tibble(trees = 1000, min_n = 11)
  )
best_model_workflow
```
:::
...
train_pred <- predict(training_fit, ames_train) %>%
  bind_cols(ames_train %>% select(Sale_Price)) %>%
  mutate(
    pred_price = 10^.pred,               
    mse = (pred_price - Sale_Price)^2
  )

test_pred <- predict(training_fit, ames_test) %>%
...
train_mse <- mean(train_pred$mse)
test_mse  <- mean(test_pred$mse)

error_ratio <- test_mse / train_mse
error_ratio
```

What is the ratio of the OOB prediction errors (MSE): test/train? \_\_\_\_1.919631\_\_\_\_\_\_\_
:::
