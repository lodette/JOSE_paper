...
Is there any missing data? No, there is no missing data
:::
...
lm_mod_rforest <- 
  parsnip::rand_forest(trees = tune::tune(),min_n = tune::tune() ) %>%
...
Compare to the results from Exercise 8: Compared with Exercise 8, rank_results() in Exercise 9 selects the best configuration per workflow and reports the mean RMSE across resamples, so its values differ from the single-resample .estimate seen in Exercise 8. In both outputs, the random forest workflows is the top performer; glmnet and OLS are very close behind.
:::
...
# Extract OOB MSEs safely (works if best model is ranger)
train_oob_mse <- workflows::extract_fit_parsnip(training_fit)$fit$prediction.error
test_oob_mse  <- workflows::extract_fit_parsnip(testing_fit)$fit$prediction.error

# Compute ratio of OOB prediction errors (MSE): test / train
mse_ratio <- test_oob_mse / train_oob_mse
mse_ratio
```

What is the ratio of the OOB prediction errors (MSE): test/train? 1.847965
:::
